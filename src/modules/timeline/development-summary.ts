...
import { readRawPayload, toRecord, readNumericValue } from "./raw-payload-helpers";
...
export function extractEmbeddedJsonValue(value: string) {
...
export function readCountLike(value: unknown) {
...
export function derivePullRequestStatusFromCounters(record: Record<string, unknown>) {
...
export function hasDevelopmentSummary(summary: DerivedDevelopmentSummary) {
  return summary.pullRequestCount > 0 || summary.commitCount > 0;
}

export function mergeDevelopmentSummaries(
...
export function readSummaryNode(value: unknown): DerivedDevelopmentSummary | null {
...
export function readTargetsSummary(value: unknown): DerivedDevelopmentSummary | null {
  const record = toRecord(value);
  const targets =
...
export function looksLikeDevelopmentFieldValue(value: unknown) {
  if (typeof value === "string") {
...
export function readDevelopmentFieldValue(rawPayload: Prisma.JsonValue | null) {
