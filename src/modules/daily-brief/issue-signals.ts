...
type DailyBriefPullRequestStatus = "OPEN" | "MERGED" | "DECLINED" | "NONE";
...
type DerivedDevelopmentSummary = {
  pullRequestStatus: DailyBriefPullRequestStatus;
...
function normalizePullRequestStatus(value: unknown): DailyBriefPullRequestStatus | null {
  if (typeof value !== "string") {
    return null;
  }
  ...
  if (normalized.includes("DECLINED") || normalized.includes("CLOSED")) {
    return "DECLINED";
  }
...
function derivePullRequestStatusFromCounters(record: Record<string, unknown>) {
  if ((readCountLike(record.openCount) ?? 0) > 0) {
    return "OPEN" satisfies DailyBriefPullRequestStatus;
  }

  if ((readCountLike(record.mergedCount) ?? 0) > 0) {
    return "MERGED" satisfies DailyBriefPullRequestStatus;
  }

  if ((readCountLike(record.declinedCount) ?? 0) > 0) {
    return "DECLINED" satisfies DailyBriefPullRequestStatus;
  }

  return "NONE" satisfies DailyBriefPullRequestStatus;
}

function derivePullRequestCount(record: Record<string, unknown>) {
  return (
    readCountLike(record.count) ??
    (readCountLike(record.openCount) ?? 0) +
      (readCountLike(record.mergedCount) ?? 0) +
      (readCountLike(record.declinedCount) ?? 0)
  );
}

function mergePullRequestStatus(
  current: DailyBriefPullRequestStatus,
  next: DailyBriefPullRequestStatus,
) {
...
function readSummaryNode(value: unknown): DerivedDevelopmentSummary | null {
  ...
  const pullRequestNode =
    ...
    toRecord(toRecord(record.summary)?.pullRequest);
  const pullRequestOverall = toRecord(pullRequestNode?.overall) ?? pullRequestNode;
  const commitNode =
  ...
  let pullRequestStatus: DailyBriefPullRequestStatus = "NONE";
...
export function sortChangelogHistories(rawPayload: Prisma.JsonValue | null) {
...
export function deriveComponentName(rawPayload: Prisma.JsonValue | null) {
  const payload = readRawPayload(rawPayload);
  ...
  const componentHistories = sortChangelogHistories(rawPayload).filter((history) =>
    history.items?.some(
